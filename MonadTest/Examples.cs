...
namespace MonadTest
{
    public class Examples
    {
        public Examples()
        {
            ...
            //readonly IO<Option<int>>[] monadList = new IO<Option<int>>[] { IO<Option<int>>.Of(() => Option<int>.Some(3)), IO<Option<int>>.Of(() => Option<int>.Some(3)), IO<Option<int>>.Of(() => Option<int>.Some(3)) };
            IO<Option<int>>[] monadList = new IO<Option<int>>[]
            {
            ...
            Option<int> h = monadList.Fold(Option<int>.Some(0), (_, y) => _.FlatMap(f => y.Map(z => f + z)));
            Option<int> result = h;
            ...
            Console.WriteLine(result.GetOrElse(1));


            Option<int> resultt = monadList.Fold(
                Option<int>.Some(0),
                (acc, ioValue) => acc.FlatMap(f => ioValue.Map(z => f + z))
            );
